Public Class clsString
    Public Shared Function normalize_Filename(ByRef opfer$) As String
        Try
            opfer = opfer$.Replace("'", "_")
            opfer = opfer$.Replace(" ", "_")
            opfer = opfer$.Replace(",", "_")
            opfer = opfer$.Replace(";", "_")
            opfer = opfer$.Replace("<", "_")
    ...
    Public Shared Function noWhiteSpace(ByRef meintext$) As String
...
ende:
    ...
    Public Shared Function gemeinsames_element(ByVal layers$(), ByVal ug$()) As Boolean
        ...
        Try
            ...
            Return False
        Catch ex As Exception
            ' Dim FehlerHinweis$ = String.Format(", Fehler: {0}{1} {0}{2} {0}{3} ", vbCrLf, ex.Message, ex.StackTrace, ex.Source)
            'mylog.log("ERROR /fehler in gemeinsames_element" & FehlerHinweis$)
            'mylog.log(FehlerHinweis)
            Return False
        End Try
    End Function
    Public Shared Function normalizeSuchdbStrasse(ByRef s1$) As String
        Dim s$
        'alles lowercase
        ...
        End Try
    End Function
    Public Shared Function normalizeGemeindenamen(ByVal s1$) As String
        Dim s$ = (s1$.ToLower.Trim)
        s$ = s$.Replace("ß", "ss")
        s$ = s$.Replace("str.", "strasse")
        s$ = s$.Replace("str ", "strasse")
        s$ = s$.Replace(" ", "")
...
#Region " Layerstring "
    ...
    Public Shared Function nodoubleStrings(ByRef ebenen_rein$, ByVal delim As Char) As String
        ...
        Dim test_string = ""
        Dim einal$() = ebenen_rein.Split(delim)
        'b1& = UBound(einal)
